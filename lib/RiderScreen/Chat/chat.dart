...
class _ChatingPageState extends ConsumerState<ChatingPage> {
  ...
  void _addMessages(List<dynamic> raw) {
    final List<Map<String, dynamic>> incoming = raw.map((e) => e as Map<String, dynamic>).toList();
...
class _ChatingPageState extends ConsumerState<ChatingPage> {
  ...
  // Add messages with date grouping (preserves old messages)
  void _addMessages(List<dynamic> raw, {bool clearFirst = false}) {
    ...
    incoming.sort((a, b) => DateTime.parse(a['createdAt']).compareTo(DateTime.parse(b['createdAt'])));
