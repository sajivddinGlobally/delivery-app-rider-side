...
class _MapRequestDetailsPageState extends State<MapRequestDetailsPage> {
  ...
  Future<void> _fetchRoute() async {
    ...
    if (widget.pickupLat == null || widget.pickupLong == null) return;

    String origin = '${_currentLatLng!.latitude},${_currentLatLng!.longitude}';
    String pickupDest = '${widget.pickupLat},${widget.pickupLong}';
    ...
    double dist1 = 0;
    int time1 = 0;
    String url1 = 'https://maps.googleapis.com/maps/api/directions/json?origin=$origin&destination=$pickupDest&key=$apiKey';
    try {
      final response1 = await http.get(Uri.parse(url1));
      if (response1.statusCode == 200) {
        final data1 = json.decode(response1.body);
        if (data1['status'] == 'OK' && data1['routes'].isNotEmpty) {
          String encodedPolyline1 = data1['routes'][0]['overview_polyline']['points'];
          points1 = _decodePolyline(encodedPolyline1);
          if (data1['routes'][0]['legs'].isNotEmpty) {
            var leg1 = data1['routes'][0]['legs'][0];
            dist1 = leg1['distance']['value'] / 1000.0;
            time1 = leg1['duration']['value'] ~/ 60;
          }
        }
      }
    } catch (e) {
      print('Error fetching route to pickup: $e');
    }
    ...
    int time2 = 0;
    if (widget.dropLat != null && widget.droplong != null) {
      String dropDest = '${widget.dropLat},${widget.droplong}';
      String url2 = 'https://maps.googleapis.com/maps/api/directions/json?origin=$pickupDest&destination=$dropDest&key=$apiKey';
      try {
        final response2 = await http.get(Uri.parse(url2));
        if (response2.statusCode == 200) {
          final data2 = json.decode(response2.body);
          if (data2['status'] == 'OK' && data2['routes'].isNotEmpty) {
            String encodedPolyline2 = data2['routes'][0]['overview_polyline']['points'];
            points2 = _decodePolyline(encodedPolyline2);
            if (data2['routes'][0]['legs'].isNotEmpty) {
              var leg2 = data2['routes'][0]['legs'][0];
              dist2 = leg2['distance']['value'] / 1000.0;
              time2 = leg2['duration']['value'] ~/ 60;
            }
          }
        }
      } catch (e) {
        print('Error fetching route to drop: $e');
      }
    }
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                ...
                if (otp.length == 4) {
                  // Here you can validate OTP with backend
  ...
  Widget build(BuildContext context) {
    ...
    final senderName = customer != null ? '${customer.firstName ?? ''} ${customer.lastName ?? ''}' : 'Unknown Sender';
    final deliveries = customer?.completedOrderCount ?? 0;
    final rating = customer?.averageRating ?? 0;
    final phone = customer?.phone ?? '';
    final packageType = packageDetails?.fragile == true ? 'Fragile Item' : 'Electronics/Gadgets';
        ...
        children: [
          ...
          GoogleMap(
            ...
            onMapCreated: (controller) {
              ...
              if (!_routeFetched && (widget.pickupLat != null || widget.dropLat != null)) {
                _routeFetched = true;
                _fetchRoute();
              }
            },
          ...
          Positioned(
            ...
            child: Container(
              ...
              width: 340.w,
              height: 300.h, // Increased height to accommodate more content
              decoration: BoxDecoration(
                    ...
                    GestureDetector(
                      ...
                      child: Text.rich(
                        TextSpan(
                          children: [
                            TextSpan(
                              text: "Recipient: ${dropoff?.name ?? 'Unknown'}",
                              style: GoogleFonts.inter(
                                fontSize: 12.sp,
                                fontWeight: FontWeight.w400,
                                color: Color(0xFF545454),
...
class _MapRequestDetailsPageState extends State<MapRequestDetailsPage> {
  ...
  //   String origin = '${_currentLatLng!.latitude},${_currentLatLng!.longitude}';
  //   String pickupDest = '${widget.pickupLat},${widget.pickupLong}';
  //   String apiKey = 'AIzaSyYourActualKeyHere';  // <- REPLACE THIS WITH YOUR ACTUAL GOOGLE DIRECTIONS API KEY!
  //
  //   // Fetch route to pickup
  //   List<LatLng> points1 = [];
  //   double dist1 = 0;
  //   int time1 = 0;
  //   String url1 = 'https://maps.googleapis.com/maps/api/directions/json?origin=$origin&destination=$pickupDest&key=$apiKey';
  //   print('Fetching route to pickup: $url1');  // Debug log
  //   try {
  //     final response1 = await http.get(Uri.parse(url1));
  //     print('Response status for pickup: ${response1.statusCode}');  // Debug
  //     if (response1.statusCode == 200) {
  //       final data1 = json.decode(response1.body);
  //       print('API Response for pickup: ${data1['status']}');  // Debug (e.g., OK or REQUEST_DENIED)
  //       if (data1['status'] == 'OK' && data1['routes'].isNotEmpty) {
  //         String encodedPolyline1 = data1['routes'][0]['overview_polyline']['points'];
  //         points1 = _decodePolyline(encodedPolyline1);
  //         print('Decoded ${points1.length} points for pickup route');  // Debug
  //         if (data1['routes'][0]['legs'].isNotEmpty) {
  //           var leg1 = data1['routes'][0]['legs'][0];
  //           dist1 = leg1['distance']['value'] / 1000.0;
  //           time1 = leg1['duration']['value'] ~/ 60;
  //         }
  //       } else {
  //         print('API error for pickup: ${data1['status']} - ${data1['error_message'] ?? 'No details'}');
  //       }
  //     } else {
  //       print('HTTP error for pickup: ${response1.body}');
  //     }
  //   } catch (e) {
  ...
  //     String dropDest = '${widget.dropLat},${widget.droplong}';
  //     String url2 = 'https://maps.googleapis.com/maps/api/directions/json?origin=$pickupDest&destination=$dropDest&key=$apiKey';
  //     print('Fetching route to drop: $url2');  // Debug
  //     try {
  //       final response2 = await http.get(Uri.parse(url2));
  //       print('Response status for drop: ${response2.statusCode}');  // Debug
  //       if (response2.statusCode == 200) {
  //         final data2 = json.decode(response2.body);
  //         print('API Response for drop: ${data2['status']}');  // Debug
  //         if (data2['status'] == 'OK' && data2['routes'].isNotEmpty) {
  //           String encodedPolyline2 = data2['routes'][0]['overview_polyline']['points'];
  //           points2 = _decodePolyline(encodedPolyline2);
  //           print('Decoded ${points2.length} points for drop route');  // Debug
  //           if (data2['routes'][0]['legs'].isNotEmpty) {
  //             var leg2 = data2['routes'][0]['legs'][0];
  //             dist2 = leg2['distance']['value'] / 1000.0;
  //             time2 = leg2['duration']['value'] ~/ 60;
  //           }
  //         } else {
  //           print('API error for drop: ${data2['status']} - ${data2['error_message'] ?? 'No details'}');
  //         }
  //       } else {
  //         print('HTTP error for drop: ${response2.body}');
  //       }
  //     } catch (e) {
  //       print('Exception fetching route to drop: $e');
  //     }
  //   }
  //
  //   List<LatLng> allPoints = [...points1, ...points2];
  ...
  //     _routePoints = allPoints;
  //
  //     // Show error if no routes fetched
  //     if (points1.isEmpty && points2.isEmpty) {
  //       ScaffoldMessenger.of(context).showSnackBar(
  //         const SnackBar(content: Text('Failed to load route. Check API key or internet.')),
  //       );
  //     }
  //   });
  //
  ...
  Future<void> _fetchRoute() async {
    ...
    if (_currentLatLng != null && widget.pickupLat != null && widget.pickupLong != null) {
      points1 = _generateMockPolyline(_currentLatLng!, LatLng(widget.pickupLat!, widget.pickupLong!), 20);  // 20 points for smooth line
    }
    ...
    if (widget.dropLat != null && widget.droplong != null) {
      points2 = _generateMockPolyline(LatLng(widget.pickupLat!, widget.pickupLong!), LatLng(widget.dropLat!, widget.droplong!), 25);
    }
    ...
    print('Mock route loaded: ${points1.length} points to pickup, ${points2.length} to drop');  // Debug
  }
